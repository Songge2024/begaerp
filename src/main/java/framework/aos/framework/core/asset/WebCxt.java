 ...
 */
public class WebCxt {
	private static Logger logger=LoggerFactory.getLogger(WebCxt.class);
	/**
	...
	public static Dto getParamAsDto(HttpServletRequest request) {
		...
		while (keyIterator.hasNext()) {
			...
			String value = map.get(key).length > 1 ? Arrays.toString(map.get(key)) : map.get(key)[0];
			dto.put(key, value);
		}
		if(dto.containsKey("api_params")){
			Dto apiParamsDto=AOSJson.fromJson(dto.getString("api_params"), HashDto.class);
			dto.remove("api_params");
			dto.putAll(apiParamsDto);
		}
		return dto;
	}
