...
public class IdService {
	 ...
	 * @return
	 */
	public String code(String codeType, String timeFormat, String maxIncr){
		if (AOSUtils.isEmpty(codeType)) {
			throw new AOSException("idType不能为空，请分配ID类型标识。");
		}
		Jedis jedis = JedisUtil.getJedisClient();
		String id = null;
		String IDSET = "CODESET";
		String key = AOSCons.KEYS.CODE +"-"+ codeType;
		timeFormat = AOSUtils.isEmpty(timeFormat) ? "yyyyMMdd" : timeFormat;
	 ...
	 * @return
	 */
	public  String code(String idType){
		return code(idType, null, null);
	}
	/**
	 * 返回基于数据表生成的ID-数据库无关性
	 * 
