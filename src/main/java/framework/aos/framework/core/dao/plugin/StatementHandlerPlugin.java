package aos.framework.core.dao.plugin;
...
import aos.framework.core.dao.asset.MybatisReflectUtils;
import aos.framework.core.dao.plugin.dialect.Dialect;
import aos.framework.core.dao.plugin.dialect.db.DefaultDialect;
import aos.framework.core.dao.plugin.dialect.db.OracleDialect;
...
import aos.framework.core.utils.AOSDataFilter;
...
public class StatementHandlerPlugin implements Interceptor {
	...
	private static Logger log = LoggerFactory.getLogger(StatementHandlerPlugin.class);
	...
	public Object intercept( Invocation pInvocation) throws Throwable {
		...
		BaseStatementHandler delegate = (BaseStatementHandler) MybatisReflectUtils.getValueByFieldName(statementHandler, "delegate");
		MappedStatement mappedStatement = (MappedStatement) MybatisReflectUtils.getValueByFieldName(delegate, "mappedStatement");
		
		BoundSql boundSql = delegate.getBoundSql();
		String rawSql = boundSql.getSql();
		String sqlPath=mappedStatement.getResource();
		...
		if (StringUtils.startsWithIgnoreCase(rawSql, "SELECT")) {
			update = true;
			sqlType="SELECT";
		}
		rawSql=AOSDataFilter.beforeFilter(sqlType,sqlPath,sqlid, rawSql, boundSql.getParameterObject());
		MybatisReflectUtils.setValueByFieldName(boundSql, "sql", rawSql);
