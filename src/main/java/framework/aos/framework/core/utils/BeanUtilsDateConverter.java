...
public class BeanUtilsDateConverter implements Converter {
	...
	@Override
	public <T> T convert(Class<T> paramClass, Object paramObject) {
		if (AOSUtils.isEmpty(paramObject)) {
			return null;
		}
		try {
			SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			return (T) df.parse(paramObject.toString());
			...
			try {
				...
				try {
					...
					try {
						...
						return (T) dfParse.parse(paramObject.toString());
					} catch (ParseException e3) {
						logger.warn("对象间日期类型属性复制时由于格式问题解析失败，属性值【{}】复制失败。", paramObject.toString());
						e3.printStackTrace();
					}
				}
			}
