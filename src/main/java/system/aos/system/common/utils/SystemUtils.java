...
public class SystemUtils {
	...
	/**
	 * 将平面树形节点转换为立体结构 -----------用于一次性加载所有节点
	 * @param list
	 * @param parentTreeNode
	 * @return parentTreeNode
	 * @author remexs
	 */
	public static TreeNode transTreeNodesToOne(List<TreeNode> list,TreeNode parentTreeNode) {
		Iterator<TreeNode> iterator = list.iterator();
		while (iterator.hasNext()) {
			TreeNode next = iterator.next();
			if (next.getParentId().equals(parentTreeNode.getId())) {
				parentTreeNode.appendChild(next);
				transTreeNodesToOne(list, next);
			}
		}
		return parentTreeNode;
	}
