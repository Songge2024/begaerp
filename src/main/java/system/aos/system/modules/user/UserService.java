...
public class UserService {
	 ...
	 * @return
	 */
	public void updateCheckUser(HttpModel httpModel) {
		Dto outDto = Dtos.newOutDto();
		Dto inDto = httpModel.getInDto();
		AosUserPO aosUserPO = new AosUserPO();
		aosUserPO.copyProperties(inDto);
			aosUserDao.updateByKey(aosUserPO);
			outDto.setAppMsg("成功添加考核人。");
		httpModel.setOutMsg(AOSJson.toJson(outDto));
	}
	/**
	 * 查询自定义下拉组件数据
	 ...
	 * @return
	 */
	public void listComboBoxcheckUserId(HttpModel httpModel) {
		Dto inDto = httpModel.getInDto();
		List<Dto> list = sqlDao.list("User.listComboBoxcheckUserId", inDto);
	...
	public void deleteUser(HttpModel httpModel) {
		...
		for (String id : selectionIds) {
			...
			aosUserDao.updateByKey(aosUserPO);
			sqlDao.update("User.deleteByUserID", Integer.valueOf(id));
		}
	 ...
	 * @return
	 */
	public void listDeleteUsers(HttpModel httpModel) {
		Dto qDto = httpModel.getInDto();
		AosOrgPO aosOrgPO = httpModel.getUserModel().getAosOrgPO();
		if (AOSUtils.isEmpty(qDto.getString("org_id"))) {
			qDto.put("org_id", aosOrgPO.getId());
		...
		qDto.put("is_del", SystemCons.IS.YES);
		List<Dto> userDtos = sqlDao.list("User.listUsersDeletePage", qDto);
		for (Dto dto : userDtos) {
			String roles_ = "";
			List<AosRolePO> aosRolePOs = sqlDao.list("User.listRolesOfUser", dto.getString("id"));
			for (AosRolePO aosRolePO : aosRolePOs) {
	 ...
	 */
	@Transactional
	public void undeleteUser(HttpModel httpModel) {
		String[] selectionIds = httpModel.getInDto().getRows();
		for (String id : selectionIds) {
			// 自己不能删除自己
			if (StringUtils.equals(httpModel.getUserModel().getId().toString(), id)) {
			...
			aosUserPO.setIs_del(SystemCons.IS.NO);
			aosUserDao.updateByKey(aosUserPO);
			sqlDao.update("User.undeleteByUserID", Integer.valueOf(id));
			
		}
		httpModel.setOutMsg("恢复用户数据成功。");
	}
}
