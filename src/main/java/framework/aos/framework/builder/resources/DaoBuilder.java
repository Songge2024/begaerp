...
public class DaoBuilder {
	...
	public static void buildPO(Dto inDto) {
		...
		tableVO.setPrefix(inDto.getString("prefix"));
		...
		try {
			String outPath = inDto.getString("outPath") + "//dao//po//";
			FileUtils.forceMkdir(new File(outPath));
			FileOutputStream out = new FileOutputStream(outPath + tableDto.getString("upclassname") + "PO.java");
			out.write(writer.toString().getBytes());
			out.close();
			if (log.isInfoEnabled()) {
				log.info("PO文件[" + packageString + "dao.po." + tableDto.getString("upclassname") + "PO.java]生成成功。");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void buildJavaDao(Dto inDto) {
		...
		try {
			String outPath = inDto.getString("outPath") + "//dao//";
			FileUtils.forceMkdir(new File(outPath));
			FileOutputStream out = new FileOutputStream(outPath + tableDto.getString("upclassname") + "Dao.java");
			out.write(writer.toString().getBytes());
			out.close();
			if (log.isInfoEnabled()) {
				log.info("Dao Java文件[" + packageString + ".dao." + tableDto.getString("upclassname") + "Dao.java]生成成功。");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void buildXmlDao(Dto inDto) {
		...
		try {
			String outPath = inDto.getString("outPath") + "//dao//sqlmap//";
			FileUtils.forceMkdir(new File(outPath));
			FileOutputStream out = new FileOutputStream(outPath + tableDto.getString("upclassname") + "Dao.xml");
			out.write(writer.toString().getBytes());
			out.close();
			if (log.isInfoEnabled()) {
				log.info("Dao Xml文件[" + packageString + ".dao.sqlmap." + tableDto.getString("upclassname") + "Dao.xml]生成成功。");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void buildJavaService(Dto inDto) {
		...
		vmDto.put("sysdate", AOSUtils.getDateTimeStr());
		List<Dto> columnDtos = BuilderUtils.convertColumnVO(columnVOs);
		StringWriter writer = VelocityHelper.mergeFileTemplate(Resources.SERVICE_VM, vmDto);
		try {
			String outPath = inDto.getString("outPath") + "//service//";
			FileUtils.forceMkdir(new File(outPath));
			FileOutputStream out = new FileOutputStream(outPath + tableDto.getString("upclassname") + "Service.java");
			out.write(writer.toString().getBytes());
			out.close();
			if (log.isInfoEnabled()) {
				log.info("Dao Java文件[" + packageString + ".service." + tableDto.getString("upclassname") + "Service.java]生成成功。");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static void buildView(Dto inDto) {
		TableVO tableVO = (TableVO) inDto.get("tableVO");
		List<ColumnVO> columnVOs = (List<ColumnVO>) inDto.get("columnVOs");
		tableVO = BuilderUtils.isAutoIncreamentColAsPK(columnVOs, tableVO);
		tableVO.setPrefix(inDto.getString("prefix"));
		...
		vmDto.put("author", inDto.getString("author"));
		vmDto.put("sysdate", AOSUtils.getDateTimeStr());
		String[] views = inDto.getString("views").split(",");
		for(String view:views){
			StringWriter writer = VelocityHelper.mergeFileTemplate(Resources.getBasePath()+"views/"+view, vmDto);
			try {
				String outPath = inDto.getString("outPath") + "/views/";
				FileUtils.forceMkdir(new File(outPath));
				FileOutputStream out = new FileOutputStream(outPath + tableDto.getString("lowclassname") + "_" + StringUtils.substringBefore(view, ".") + ".jsp");
				out.write(writer.toString().getBytes());
				out.close();
				if (log.isInfoEnabled()) {
					log.info("Dao Xml文件[" + packageString + ".view." + tableDto.getString("lowclassname") + ".jsp]生成成功。");
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
